...
(defun at-least-one-color (n c k)
  (cond ( (< k 1) nil)
        ( (< k c) nil)
	( (= k c) (list (+ c (* k (- n 1)))))
	( t (cons (+ c (* k (- n 1))) (at-least-one-color n (+ 1 c) k)))
	)
  )
...
(defun at-most-one-color (n c k)
  (at-most-helper n c k c)
  )

; a helper function that takes one more parameter as the pointer
(defun at-most-helper(n c k h )
  (cond ( ( > h k) NIL)
	( t (cons (remove-one-index n c k h) (at-most-helper n c k (+ 1 h))))
	)
  )
	

; remove one index from the clause and make the rest negative form
(defun remove-one-index(n c k x)
  (cond ( (= c x) (generate-negate-list (at-least-one-color n (+ 1 c) k)))
	( t (cons (- 0 (node2var n c k)) (remove-one-index n (+ 1 c) k x)))
	)
  )
  

; change the element of a list to negative form
; i.e: (1,2,3) -> (-1,-2,-3)
(defun generate-negate-list (l)
  (cond ( (NULL l) nil)
	( t (cons (- 0 (car l)) (generate-negate-list (cdr l))))
	)
  )
...
(defun gen-edge-helper(x y k h)
  (cond ( (> h k) NIL)
	( t  (cons (list (- 0 (node2var x h k)) (- 0 (node2var y h k))) (gen-edge-helper x y k (+ 1 h))))
	)
  )
...
(defun generate-edge-clauses (e k)
  (cond ( (< k 1) NIL)
	( t  (let( ( x (car e)) ( y (cadr e)))
	      (gen-edge-helper x y k 1)
	      )
	      )
	    )
	)
